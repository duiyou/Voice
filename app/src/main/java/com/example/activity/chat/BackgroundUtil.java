...
import com.example.util.RobotUtil;
...
public class BackgroundUtil {
    ...
    public interface BackgroundUtilListener{
        ...
        //得到语义的domain
        void domainRecognize(String doaminStr);
        //结束说话
        void endSpeech();
        //完成语音识别
        void wellRecognize();
        //机器人回复结果
        void wellRobotReply(String reply);
    }
    ...
    public static synchronized BackgroundUtil getInstance(ChatActivity chatActivity){
        if(instance == null)instance = new BackgroundUtil(chatActivity);
        return instance;
    }
    //私有构造器
    private BackgroundUtil(ChatActivity chatActivity){
    ...
    public void initSpeechRecognitionUtil(){
        speechRecognitionUtil = SpeechRecognitionUtil.getInstance(chatActivity, new MySpeechRecognitionListener());
    }
        ...
        public void onEvent(String name, String params, byte[] data, int offset, int length) {
            ...
            */
            ...
            if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_READY)) {
                ...
                switch(resultType){
                    //得到最后结果时
                    case SpeechRecognitionUtil.VALUE_FINAL_RESULT:
                        ...
                        break;
                    //得到语义理解数据时
                    case SpeechRecognitionUtil.VALUE_UNDERSTAND_RESULT:
                        //去获取domain
                        Log.i(TAG, "onEvent: 语义理解参数：" + new String(data, offset, length));
                        String domainResult
                                = SpeechRecognitionUtil.getDomain(new String(data, offset, length));
                        Log.i(TAG, "onEvent: 语义结果：" + domainResult);
                        if (mListener != null) {
                            mListener.domainRecognize(domainResult);
                        }
                        break;
                }
    ...
    public void initSpeechSynthesisUtil(){
        speechSynthesisUtil = SpeechSynthesisUtil.getInstance(chatActivity);
    ...
    private void initRobotUtil(){
        robotUtil = new RobotUtil();
        robotUtil.setRobotReplyListener(new MyRobotReplyListener());
    }

    /**
     * 自定义机器人监听类
     */
    class MyRobotReplyListener implements RobotUtil.RobotReplyListener {
        ...
        public void wellRobotReply(String robotReply) {
            //告诉界面，机器人回复了
            if(mListener != null) {
                mListener.wellRobotReply(robotReply);
            }
        }
    }
    ...
    public void sendMessage(String message){
        if(robotUtil != null) {
            robotUtil.robotReply(message);
        } else {
            Log.i(TAG, "sendMessage: 发送聊天信息错误");
        }
    }
}
